...
export default function ProfileTransaction({
  transaction,
}: ProfileTransactionTypes): React.ReactNode {
  const item = transaction?.countItem ?? 0;
  const price = Number(transaction?.product?.price).toLocaleString("id-ID", {
    style: "currency",
    currency: "IDR",
  });
  const subTotal = (item * Number(transaction?.product?.price)).toLocaleString(
    "id-ID",
        ...
        <VStack w={"full"}>
          <Box w={"full"}>
            ...
            <Text color={"brand.active"}>
              {detailDatePost(`${transaction?.product?.createdAt}`)}
            </Text>
